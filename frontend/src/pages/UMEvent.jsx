// === CLEANED UMevent.jsx — Focus on Registered Events ===
...
export default function UMevent() {
  const BACKEND_URL =
  ...
  const fetchEvents = async () => {
    try {
      ...
      const registered = data.events
        ...
        .filter(
          (event) =>
            event.internalParticipants?.includes(userData._id) ||
            event.externalParticipants?.includes(userData._id)
        )
        .map((event) => ({ ...event, isRegistered: true }));
  ...
  const filteredRegistered = registeredEvents.filter(matchesFilter);

  return showfeedbackform ? (
    <UserFeedbackForm
      eventId={showfeedbackform}
      onClose={setShowfeedbackform}
    />
  ) : (
    <div id="container" className="flex column">
      ...
      <div className="section flex column gap-12">
        <p className="sectionHeading">My Events</p>
        ...
        <div className="flex row wrap start">
          {filteredRegistered.length === 0 ? (
            <p className="mutedText marg-12">No registered events.</p>
          ) : (
            filteredRegistered.map(renderCard)
          )}
        </div>
      </div>
    </div>
